...
export default function AddonsClient({
  ...
  const fetchPackages = async (
    ...
    try {
      const params = new URLSearchParams({
        ...
        search,
        status,
        packageId: pkgFilter === 'ALL' ? '' : pkgFilter
      });
  ...
  useEffect(() => {
    const delayDebounce = setTimeout(() => {
      fetchPackages(page, limit, searchTerm, statusFilter, packageFilter);
    }, 300);

    return () => clearTimeout(delayDebounce);
  }, [page, limit, searchTerm, statusFilter, packageFilter]);
  ...
  const handleToggleStatus = async (id: string, currentStatus: boolean) => {
    try {
      ...
      if (res.ok) {
        ...
        showToast(`Add-on successfully ${currentStatus ? 'deactivated' : 'activated'}.`, 'success');
      } else {
        const errorData = await res.json().catch(() => ({}));
        showToast(errorData.error || 'Failed to update add-on status.', 'error');
      }
    } catch (error) {
      showToast('Failed to update add-on status.', 'error');
    }
  };
  ...
  return (
    <div className="space-y-6 font-poppins">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-semibold tracking-tight text-slate-900">Add-on Packages</h1>
          <p className="text-neutral-500 mt-1">Manage all your travel add-ons</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-white p-4 rounded-xl border border-slate-200/80 shadow-[0_2px_8px_rgba(0,0,0,0.02)]">
        ...
        <div>
          <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider block mb-1">Package</label>
          <select
            value={packageFilter}
            onChange={(e) => {
      ...
      <TableContainer className="relative min-h-[150px]">
        {loading && (
          <div className="absolute inset-0 bg-white/60 backdrop-blur-[1px] z-10 flex items-center justify-center transition-all">
            <div className="flex items-center gap-2 bg-white/95 px-4 py-2.5 rounded-full shadow-md border border-slate-100">
              ...
              <span className="text-xs font-semibold text-slate-700">Loading add-ons...</span>
            </div>
          </div>
        )}
        ...
        {packages.length === 0 ? (
          <div className="text-center py-12 text-slate-500 border border-dashed border-slate-200 rounded-xl bg-slate-50/50">
            No add-ons found matching the current filters.
          </div>
        ) : (
          <Table>
            <TableHeader>
              ...
              {packages.map((pkg) => (
                <TableRow key={pkg.id}>
                  ...
                  <TableCell className="font-medium text-slate-700">
                    {pkg.addonFor && pkg.addonFor.length > 0
                      ? pkg.addonFor.map((af: any) => af.package?.title).filter(Boolean).join(', ')
                      : 'None'}
                  </TableCell>
                  ...
                  <TableCell className="text-right space-x-1.5">
                    ...
                    <ViewButton href={`/admin/addons/${pkg.id}/view`} />
                    <EditButton href={`/admin/addons/${pkg.id}`} />
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-white p-4 rounded-xl border border-slate-200/80 shadow-sm">
        <div className="text-sm font-medium text-slate-500">
          ...
          <span className="font-semibold text-slate-800">{total}</span> add-ons
        </div>
